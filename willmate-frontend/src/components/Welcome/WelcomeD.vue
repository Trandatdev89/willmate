  ...
  <div class="container">
    <form @submit="handleSubmit" class="container__submit">
      <div class="container__item">
        ...
        <div
          class="error-message"
          :class="{ show: errorValidate.username }"
          v-show="errorValidate.username"
        >
      ...
      <div class="container__item">
        ...
        <div
          class="error-message"
          :class="{ show: errorValidate.password }"
          v-show="errorValidate.password"
        >
      ...
      <div class="container__item">
        ...
        <div
          class="error-message"
          :class="{ show: errorValidate.retypePassword }"
          v-show="errorValidate.retypePassword"
        >
          {{ errorValidate.retypePassword }}
        </div>
      </div>
      <div class="container__item">
        <div class="age" for="age">Age</div>
        <input type="number" id="age" v-model="dataForm.age" />
        <div class="error-message" :class="{ show: errorValidate.age }" v-show="errorValidate.age">
          {{ errorValidate.age }}
        </div>
      </div>
      <div class="container__item">
...
const validateField = () => {
  if (
    !validateFieldEmptyOrNull(dataForm.username) ||
  ...
  if (
    !(
      validateUserName(dataForm.username) ||
      validatePassword(dataForm.password, dataForm.retypePassword) ||
      validateAge(dataForm.age)
    )
  ) {
    return
  }
...
const validateUserName = (username: string) => {
  if (username.length < 6 && username === username.toLowerCase()) {
    errorValidate.username =
      'Username must more than six character and less than one character uppercase'
    return false
  }
  return true
}
